...
export const Header = () => {
  return (
    <div className="w-full flex justify-center">
      <div className="bg-white w-fit shadow shadow-[#f0f0f0] rounded-[40px] px-[150px] py-3">
        <div className="flex justify-between items-center gap-[64px]">
          {/* Logo */}
          <Link href="/">
            <Image src="/foso-logo.svg" alt="FOSO Logo" width="134" height="55" />
          </Link>
          ...
          <nav>
            <ul className="flex items-center gap-2 text-sm">
              ...
              <li>
                <Dropdown
                  items={["Giải pháp 1", "Giải pháp 2"].map((title, index) => (
                    <Link key={index} className="py-2" href="#">
                      {title}
                    </Link>
                  ))}>
              ...
              <li>
                <Dropdown
                  items={["Tài nguyên 1", "Tài nguyên 2"].map((title, index) => (
                    <Link key={index} href="#">
                      {title}
                    </Link>
                  ))}>
          ...
          <div className="flex gap-2.5">
            <Dropdown
              items={["VI", "EN"].map((title, index) => (
                <Link key={index} href="#">
                  {title}
                </Link>
              ))}>
              <div className="flex items-center p-2 rounded-[40px] bg-linear-to-b from-[rgba(9,9,11,5%)] to-[rgba(9,9,11,10%)] border border-[rgba(9,9,11,7%)]">
